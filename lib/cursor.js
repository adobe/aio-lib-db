...
class Cursor {
  ...
  constructor(axiosClient, operationDefinition, tenantId, collection, cursorId, firstBatch = [], options = {}) {
    ...
    // Freeze the public properties
    this.#makeImmutable('id')
    this.#makeImmutable('collectionName')
    this.#makeImmutable('tenantId')
    this.#makeImmutable('operation')
    this.#makeImmutable('options')
  }
  ...
  // Helper to make a property read-only
  #makeImmutable(property) {
