const Cursor = require("../DbCursor")
const { apiPost } = require("../../utils/apiRequest")

/**
 * Make a post request to the DB Proxy Service collection API and return the result
 *
 * @param {Db} db
...
async function find(db, collectionName, filter, options = {}) {
  const data = await postCollectionApi(db, collectionName, 'find', { filter: filter }, options)
  return new Cursor(
    db,
 ...
 * @throws {DbError}
 */
async function aggregate(db, collectionName, pipeline, options = {}) {
  const data = await postCollectionApi(db, collectionName, 'aggregate', { pipeline: pipeline }, options)
  return new Cursor(
    db,
