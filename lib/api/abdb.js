...
const PING_URL = `${ENDPOINT_URL}/v1/db/ping`
 ...
 * @throws {AbdbError}
 */
async function ping(axiosClient) {
  try {
    const res = await axiosClient.get(PING_URL)
    if (!res.data.success) {
      throw new AbdbError(`ping failed: ${res.data.message}`)
    }
    return res.data.data
  } catch (err) {
    if (err.response?.data) {
      throw new AbdbError(`ping failed: ${err.response.data.message}`, { cause: err })
    }
    throw err
  }
}
