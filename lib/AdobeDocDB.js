...
class AdobeDocDB {
   ...
   */
  async connect() {
    const connectionInfo = await connectToDb(this.tenantId, this.axiosClient)
    this.connected = true
  ...
  async dbStats() {
    ...
    return await getDbStats(this.tenantId, this.axiosClient)
  }
  ...
  async close() {
    ...
    await closeConnection(this.tenantId, this.axiosClient)
    this.connected = false
