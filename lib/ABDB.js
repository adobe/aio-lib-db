...
class Abdb {
  ...
  constructor(runtimeNamespace = undefined, runtimeAuth = undefined) {
    ...
    if (!this.runtimeAuth) {
      throw new AbdbError('runtimeAuth is required in credentials or as AIO_runtime_auth in .env')
    }
    ...
    if (!this.runtimeNamespace) {
      throw new AbdbError('runtimeNamespace is required in credentials or as AIO_runtime_namespace in .env')
    }
