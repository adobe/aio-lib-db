...
class Abdb {
  /**
   * @param {Object<{"tenantId": string, "runtimeAuth": string, "runtimeNamespace": string}>} credentials
   * @hideconstructor
   */
  constructor(credentials) {
    ...
    if (!credentials['runtimeAuth']) {
      throw new Error('runtimeAuth is required credentials')
    }
    if (!/.+:.+/.test(credentials['runtimeAuth'])) {
      throw new Error("Invalid format for runtimeAuth, must be '<user>:<pass>'")
    }
    this.runtimeAuth = credentials['runtimeAuth']
