...
class Abdb {
  /**
   * @param {Object<{"tenantId": string, "runtimeNamespace": string=}>} credentials
   * @hideconstructor
   */
  constructor(credentials) {
    ...
    if (!credentials['runtimeNamespace'] && !process.env.RUNTIME_NAMESPACE) {
      throw new Error('runtimeNamespace in credentials or RUNTIME_NAMESPACE in .env is required')
    }
    this.runtimeNamespace = credentials.runtimeNamespace || process.env.RUNTIME_NAMESPACE
   ...
   */
  async connect() {
    const connectionInfo = await connectApi(this)
    this.connected = true
