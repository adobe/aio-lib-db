...
class DbCursor extends Readable {
  /** @type DbBase */
  #db
  /** @type string */
  #getMoreUrl
   ...
   */
  constructor(db, operationDefinition, collection, cursorId, firstBatch = [], options = {}) {
  ...
  async #getMore() {
    const data = await apiPost(this.#db, this.#getMoreUrl, { cursorId: this.#id }, this.#options)
  ...
  stream(transform = undefined) {
    // DbCursor is already a readable stream, so return itself unless a transform method is provided
    const readable = this
    if (transform) {
